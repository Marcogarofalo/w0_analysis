...
######################## rew
mt <- read_df("/home/garofalo/analysis/flow/data/out/flow_B64.dat_reweight_light_B64.dat_output")
all_obs <- Rose::get_all_corr(mt)
df <- add_corr_to_df("W_rewcOS(t)", all_obs, mt, df, print_res = FALSE, nudge = 0.1)
...
gg<-ggplot( ) + xlab("W(0.3)") +
    geom_histogram(aes(x = W, y =..density..),binwidth = -(min(W) -max(W))/7,
...
gg<-ggplot( ) + xlab("dW/dmuc (0.3)") 
 gg<- gg+    geom_histogram(aes(x = W, y =..density..),binwidth = -(min(W) -max(W))/7,
                    #breaks = seq(min(W), max(W), by = 100), 
                    colour = "black", 
                    fill = "white") +theme_bw()
