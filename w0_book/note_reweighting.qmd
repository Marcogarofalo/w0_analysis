...
$$
$r$ is computed stochastically as $r(U)= \langle r(U) \rangle_\phi= \det\left(D(\mu_f)/D(\mu_i)\right) = \int d\phi e^{w(U,\phi)}$
with $w(U,\phi)=\phi^\dagger(D(\mu_i) D^{-1}(\mu_f))\phi$.
