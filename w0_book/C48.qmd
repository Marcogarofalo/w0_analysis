...
```{r, echo=FALSE,results='asis'}
dir <- "/home/garofalo/analysis/flow/data/out/"
file <- "onlinemeas_C48.dat.not_thermalized_reweight_charm_OS_C48.dat.not_thermalized_output"
mt <- read_df(paste0(dir, file))
all_obs <- Rose::get_all_corr(mt)
df<-NULL
# df<-add_corr_to_df("M_{PS}",all_obs ,mt, df)
...
df<-add_corr_to_df("M_{PS}",all_obs ,mt, df, rename ="M_{PS}_no[0-500]a+more stat" )
df<-add_corr_to_df("f_{PS}",all_obs ,mt, df, rename ="f_{PS}_no[0-500]a+more stat")

df<-add_corr_to_df("plateau_dM/drewcOS",all_obs ,mt, df,rename="plateau_dM/drewcOS_no[0-500]aa+more stat")
df<-add_corr_to_df("plateau_df/drewcOS",all_obs ,mt,df,rename="plateau_df/drewcOS_no[0-500]aa+more stat" )
...
```{r, echo=FALSE,results='asis'}
dfl<- read.table("/home/garofalo/analysis/flow/data/out/onlinemeas_C48.dat.not_thermalized_reweight_light_OS_C48.dat.not_thermalized_history.txt", header = TRUE)
dfs<- read.table("/home/garofalo/analysis/flow/data/out/onlinemeas_C48.dat.not_thermalized_reweight_strange_OS_C48.dat.not_thermalized_history.txt", header = TRUE)
dfc<- read.table("/home/garofalo/analysis/flow/data/out/onlinemeas_C48.dat.not_thermalized_reweight_charm_OS_C48.dat.not_thermalized_history.txt", header = TRUE)
...
```{r}
df<-read.table("/home/garofalo/analysis/flow/data/out/autocorr_C48_every2_confs.txt",header = TRUE)
gg<-myggplot()
Nconfs<- 867 #df$Nbins[length(df$Nbins)]
...
```{r}
df<-read.table("/home/garofalo/analysis/flow/data/out/autocorr_C48_every4_confs.txt",header = TRUE)
gg<-myggplot()
Nconfs<- 434 #df$Nbins[length(df$Nbins)]
gg<- gg + geom_pointrange(data=df,aes(x=Nconfs/Nbins, y=Mpi, ymin=Mpi-dMpi, ymax=Mpi+dMpi))+xlim(0,20)
gg
#############################
gg<-myggplot()
...
#############################
gg<-myggplot()
gg<- gg + geom_pointrange(data=df,aes(x=Nconfs/Nbins, y=dfpi.dmul, ymin=dfpi.dmul-d.dfpi.dmul., ymax=dfpi.dmul+d.dfpi.dmul.))+xlim(0,20)+ylim(0,0.1)
gg
```