...
all_obs <- Rose::get_all_corr(mt)
df <- add_corr_to_df("M_{PS}", all_obs, mt, df, print_res = FALSE, nudge = 0.5, reshape = T, rename = "M_{PS}_LMA")
# print_full_res("w0_rewsOS", all_obs, mt)
#####
...
name <- "df_{PS}/drewsOS"
res <- get_full_res(name, all_obs, mt)
df[nrow(df) + 1, ] <- list(as.numeric(res[1, 1]), as.numeric(res[1, 2]), "rew", masses[2], paste0(name,"_LMA"))
...
all_obs <- Rose::get_all_corr(mt)
df <- add_corr_to_df("mpcac", all_obs, mt, df, print_res = T, nudge = 0.0, reshape = T)
