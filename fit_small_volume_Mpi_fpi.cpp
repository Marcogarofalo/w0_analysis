    ...
    for (int i = 0; i < 1; i++) {
        ...
        for (int n = 0; n < fit_info.N; n++) {
            ...
            for (int b = 0; b < myen.size(); b++) {
                fit_info.Nxen[n][b] = myen[b][0];  // the charm is in myen[beta][0]
            }
        }
        ...
        for (int n = 0; n < fit_info.N; n++) {
            for (int e : fit_info.Nxen[n]) {
                for (int j = 0; j < Njack; j++) {
                    ...
                    fit_info.x[0][count][j] = jackall.en[e].header.L; // mu/muliso
                }
                count++;
            }
        }
        ...
        std::string namefit = "der_" + der_name[i] + "_charm_small_volume_const";

        fit_result der_fpi_const_full = fit_all_data(argv, jackall, lhs_fun, fit_info, namefit.c_str());
        fit_info.band_range = { 23, 33 };
        print_fit_band(argv, jackall, fit_info, fit_info, namefit.c_str(), "L_a", der_fpi_const_full, der_fpi_const_full, 0, fit_info.Nxen[0][0] /* set the other variables to the first of the n*/, 1, {});
